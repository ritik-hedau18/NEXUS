...
```mermaid
flowchart TD
    A[React Frontend\nVite · TypeScript · Tailwind CSS] -->|HTTP REST / SSE| B

    subgraph Backend [Spring Boot Backend :8080]
        B[API Layer\nSpring Security · JWT Auth]
        B --> C[Auth Module]
        B --> D[Workspace Module]
        B --> E[Document Module]
        B --> F[Chat Module - RAG]
        B --> G[Search Module]
        B --> H[Summary Module]
    ...
    E -->|Apache Tika Parsing| K[Jina AI Embeddings]
    K -->|Store Vectors| L[(Qdrant\nVector Store)]

    F -->|Similarity Search| L
    F -->|LLM Generation + SSE| M[Groq API\nLLaMA 3.3-70b]
    F -->|Persist Memory| N[(PostgreSQL\nSPRING_AI_CHAT_MEMORY)]

    G -->|Semantic Search| L
    H -->|Summarization| M

    C & D & E & J -->|Read / Write| O[(PostgreSQL\nusers · workspaces\ndocuments · employees)]
```
